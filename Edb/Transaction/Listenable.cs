namespace Edb
{
    internal abstract partial class Listenable
    {
        ...
        internal abstract Task LogNotify<TKey, TValue>(object key, object value, RecordState rs, ListenerMap listenerMap, TransactionCtx ctx)
            ...
            internal override async Task LogNotify<TKey, TValue>(object key, object value, RecordState rs, ListenerMap listenerMap, TransactionCtx ctx)
            {
                switch (rs)
                {
                    ...
                    case RecordState.Changed:
                        if (m_Changed)
                            await listenerMap.NotifyChanged(m_VarName, key, value, null, ctx);
                        break;
                }
                m_Changed = false;
            }
